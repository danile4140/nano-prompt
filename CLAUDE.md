...
- 两个节点之间的唯一数据通道是 JSON 字段（`storyboard_draft` → `storyboard_assets`）。
- 因此，**任何需要在 5.3b 执行的规则，必须写进 5.3b 的系统提示词里**，不能依赖"5.3a 已经做过判断"——5.3b 根本看不到 5.3a 的推理过程。反之亦然。

这一架构约束对规则设计有重要影响：凡涉及"两节点协作"的规则，必须在各自节点的系统提示词中分别写明各自的职责边界，不得假设对方节点会补足。

**禁止在节点系统提示词内部引用另一节点的名称**：5.3a 的系统提示词里不得出现"5.3b"，5.3b 的系统提示词里不得出现"5.3a"——节点运行时根本不存在这个上下文，写了也是无效信息，还会干扰模型理解规则。修改规则时如果发现草稿里带有"5.3a/5.3b 已做过……"之类的表述，必须删掉，改写为该节点自身的完整充要判断。
